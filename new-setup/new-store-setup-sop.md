...
3. **Ordering — document order within a file; filename order across files.** The data loader's
   default path does a plain `createOrUpdate()` with **no FK existence check** (`checkFks` is
   gated behind `dummyFks`, off by default — `EntityDataLoaderImpl.handleValue`), so there is
   **no commit-boundary requirement** between parent and child records. The only thing that
   matters is that a parent record appears **before** any record that references it. A **single
   ordered file** (parent-before-child) is therefore the simplest correct approach and the
   recommended default — matching OOTB Moqui/Mantle setup data. The `A1/A2` filename prefixes in
   `gorjana-maarg` are a **defensive carryover from the legacy OFBiz component, not a framework
   requirement**; filename ordering only matters if you *choose* to split across files (a
   readability choice, never an FK necessity). See `docs/data-load-types.md`.
...
Author **one consolidated `data/SetupData.xml`** (load type `ext-seed`) with sections in
parent-before-child order. A single ordered file is the simplest correct approach — the loader
has no commit-boundary FK requirement (see Load order rule #3). Splitting into multiple files is
optional (readability only), never required.

| # | Section | Defines | New-client action |
|---|---|---|---|
| 1 | Company | `Party`/`PartyGroup` + roles (id `COMPANY`) | **Replace** with client legal entity, name, roles |
| 2 | Facilities *(NEW — see Gap #1)* | Operational warehouses/stores (`Facility`) | **Create** — neither platform nor `gorjana-maarg` ships real facilities |
| 3 | Facility groups + links | `FacilityGroup`, `ProductStoreFacilityGroup` | **Configure** to client policy |
| 4 | Product store | `ProductStore`, `ProductStoreShipmentMeth`, `ProductStoreSetting` | **Replace** store id/name, `payToPartyId`, `inventoryFacilityId`, default locale/currency/channel, approval statuses |
| 5 | Sales channels / special parties | sales-channel `Enumeration`, return special-customers | **Configure** to client policy |
| 6 | Returns | `ReturnType`, `ReturnReason`, `ReturnTypeReason` | **Configure** client return policy |
| 7 | Shipping | Carriers, `ShipmentMethodType`, rate-rule enums + rules | **Configure** client carriers & rates |
| 8 | Security groups/permissions | `SecurityGroup`, `SecurityGroupPermission` (e.g. store-manager role) | **Configure** (note Gap #2) |

Section order satisfies the FK chain: company → facilities → facility groups → product store →
settings/channels → returns/shipping. Integration mapping values follow in Phase 3 (same file, or
their own files if large).
